...
% \todo{Make this chapter-specific}

% \noindent\textbf{Our contributions.}
In this chapter, we put forth a cryptographic security definition for
Non-Interactive Proofs of Proof-of-Work\index{NIPoPoW} protocols which describes
what such a synchronization protocol must achieve. We then construct a protocol
which solves the problem and requires sending only a logarithmic number of
blocks from the chain. We construct a protocol which can synchronize recent
blocks, a \emph{suffix proof} protocol which we term the \emph{charity} NIPoPow.
We analyze the security and succinctness of our protocol. We show a simple
addition to the suffix proofs protocol which allows synchronizing any part of
the blockchain that the client may be interested in, the \emph{infix proofs}
protocol. We give formal proofs of security and succinctness, provide concrete
parameters for the implementation of our scheme, present applications beyond
superlight clients including cross-chain applications, propose a mechanism with
which our scheme can be deployed to existing cryptocurrencies without a fork.

In this chapter, we treat superblock NIPoPoWs in the \emph{synchronous} and
\emph{static difficulty} backbone setting. We relax both of these assumptions in
...
% applications~
% \cite{interledger,federated-interoperability,
%       lerner,drivechains,polkadot,tendermint}
