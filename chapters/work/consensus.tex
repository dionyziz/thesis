...
$\Theta(\log(\chain))$~\cite{popow}. Figure~\ref{fig.hierarchy3} illustrates the
...
\glsxtrnewsymbol[description={subchain of $\chain$ at the $\mu^\text{th}$ level}]{c-upchain}{$\chain\upchain^\mu$}\glsadd{c-upchain}
...
\glsxtrnewsymbol[description={$0$-level chain used to generate filtered chain $\chain$}]{c-downchain}{$\chain\downchain$}\glsadd{c-downchain}
\index{Downchain}
