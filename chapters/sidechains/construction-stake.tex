\subsection{Implementing Pegged Ledgers}
\label{sec:construction-stake}
...
%, and to follow our security argument in Section~\ref{sec:sidechain-security}.
...
%sampling slot leaders for epoch $j$ on $\Mainchain$), $\SDb_j$ will denote
%the stake distribution induced by the stake that resides on $\Sidechain$ at the same
%slot (this would be the stake distribution used for sampling slot leaders for
%epoch $j$ if $\Sidechain$ was using independent staking).
...
%distribution \emph{actually used} for leader election on $\Sidechain$ in epoch $j$ as
...
maintained. The transaction types $\mathsf{side}\-\mathsf{chain\_sup\-port}$,
...
%single sidechain which we will refer to as $\Sidechain$, but our results extend to
%multiple sidechains sharing the same mainchain.

Let $\jadopt$ denote the epoch on $\Mainchain$ when the first adoption transaction has
...
%determined by the sidechain module (we denote them as $\Sidechain.\Cleanup()$)
...
%prevents a minority of $\Mainchain$ stakeholders from continuing to support an aborted
%sidechain if they insist, in violation of the sidechain creation rules. However
%all the other stakeholders of $\Mainchain$ will consider the sidechain as failed, and
...
%removed from the ledger of $\Mainchain$. On the other hand, if the majority of stake in
%$\Mainchain$ insists on the violation of the sidechain creation rules, we observe that
%the constants $c_1,c_2$ can be effectively changed by a soft-fork of $\Mainchain$. Such
...
%result in unnecessarily cluttering the $\Mainchain$ with data related to sidechains
%that are spuriously created. }

%%Suppose the first occurrence of sidechain adoption is
%%announced by a stakeholder at epoch $j^*$ of $\Mainchain$ for some $j^* \geq 2$. Then the
...
%ledger state of $\Mainchain$. We will maintain this property: the $\Mainchain$ ledger state
...
%This is
%in contrast to a hypothetical stand-alone execution of Ouroboros  on $\Sidechain$,
...
%epoch $\rnd_j$, concretely $\rndb_{j}\defeq H(\id_\Sidechain, \rnd_{j})$.
...
Once $\Sidechain$ is initialized, cross-chain transfers to it can be made from $\Mainchain$.
...
%\begin{enumerate}[resume]
  %\item
    ...
    %additional information an address $\recAccount$ in $\Sidechain$ where the funds
    %should arrive. If the transaction is issued %in limbo
    %before $\Sidechain$ is activated,
    %then it also includes
    %an address $\recAccount'$ in $\Mainchain$ where the funds can be recovered in case
  ...
  %\item
    When $\tx_{\send}$ becomes stable in $\Mainchain$ (i.e., appears in
    ...
    %$\Sidechain$ to receive the funds into $\recAccount$ on $\Sidechain$.
    %For any
    %$\mathsf{sidechain\_send}$ transaction $\tx$ we have
    %$\eff{\Mainchain}{\Sidechain}(\tx) = \tx$ where $\tx'$ is the
    %$\mathsf{sidechain\_receive}$ transaction corresponding to $\tx$,
    %and honest slot leaders in $\Sidechain$ include these transactions into their
    %blocks (to satisfy pegging correctness).
    %The transaction $\tx_{\rec}$ credits the respective amount
    %of coins to $\recAccount$ in $\Sidechain$.
    ...
    %the $\Sidechain$ can only happen as soon as the sidechain has been created; otherwise,
  ...
  %  $\mathsf{sidechain\_recover}$ to $\Mainchain$ which returns the funds from
  %  limbo to the recovery address $\recAccount'$ in $\Mainchain$. For conciseness, we
...
%the coins can be moved from $\Mainchain$ to $\Sidechain$ in at most $8k/f$ slots ($4k/f$ to
%reliably include $\mathsf{sidechain\_send}$ into $\Mainchain$, and $4k/f$ to reliably
%include  $\mathsf{sidechain\_receive}$ into $\Sidechain$).
...
\paragraph{Withdrawing to $\Mainchain$}
  ...
  %\item
    ...
    %\begin{itemize}
      %\item
        ...
        %the total inflow of funds from $\Mainchain$ to $\Sidechain$,
      ...
      %\item
        ...
        %stakeholders following both $\Sidechain$ and $\Mainchain$, can be used to derive
        %$\SDb^*_{j'+1}$ by anyone following $\Mainchain$); and
    %\end{itemize}
    %that that certain transactions moving
    %money from $\Sidechain$ to $\Mainchain$ are pending.
    ...
    %shifting and pending transactions have taken place in $\Sidechain$.
...
into $\Sidechain$
%to $\Mainchain$ by including a $\tx_{send}$ transaction into
%$\Sidechain$
...
%examining the
%sequence of all transactions $\vec\tx$ which are included in $\Sidechain$ during those
%$R$ slots
and contains:
\begin{itemize}
    \item
...
%Each $j^\text{th}$ $\Sidechain$ epoch maintains an \emph{epoch key} $avk^{j}$ which
...
%In order for the sidechain $\Sidechain$ to advance from epoch $\epoch_j$ to
%$\epoch_{j+1}$ the $\Mainchain$ stakeholders will need to update the $avk$ that they
...
%the $\Mainchain$ with more information. For example,
...
%$\Mainchain$-maintainers (and thus also $\Sidechain$-maintainers) can
...
%that $\Comprises(\mathcal{VK}^0, avk^0)$ is \emph{true}. An honest $\Mainchain$ slot
%leader extending a blockchain not containing a commitment to a valid $avk^0$
%will include $avk^0$ into their block. Any $\Mainchain$ maintainer can verify if this
...
%i \in [2k]\})$ by any $\Sidechain$ maintainer.
...
%We need to answer the question what is the burden on $\Mainchain$ both in terms of space and in terms of verification that is incurred because of the presence $\Sidechain$. The description can be done modularly in terms of the ATMS primitive. We also need
