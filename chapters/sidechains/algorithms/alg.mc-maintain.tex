\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \On{$\MC.\NewSlot()$}
      %\Comment{$\SC$-slots coincide with $\MC$-slots}
      \Let{\slot}{\MC.\SlotIndex()}
      \CommentLine{First slot of a new epoch}
      \If{$\slot\mod R=1$}
        %\State $\SwitchMCEpoch()$
        %  \label{line:switch}
        \Let{j}{\MC.\EpochIndex()}
        %\Let{\SD_j}{\MC.\GetDistr(8k, \epoch_{j-1})}
        \Let{\SD_j}{\MC.\GetDistr(j)}
          \label{line:SD}
        \Let{\rnd_{j}}{\MC.\GetRandomness(j)}
      \EndIf
      \Let{\mathcal{C}}{\text{chains received via $\Diffuse$}}
      \CommentLine{Consensus-level validation}
      \Let{\mathcal{C}_{\valid}}{\Filter(\mathcal{C},\MC.\ValidConsLevel)}
      ...
      \Let{\ChainMC}{\MC.\PickWinningChain(\ChainMC,\mathcal{C_{\validtx}})}
      \CommentLine{Decide slot leadership based on $\SD_j$ and $\rnd_{j}$}
      \If{$\MC.\SlotLeader(U,j,sl,\SD_j,\rnd_{j})$}
        \label{line:leader}
