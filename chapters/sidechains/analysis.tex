...
is precisely $\Assum_\MC[t]\defeq\Assum_{\hm}(\MC)[t]$, while the assumption $\Assum_\SC$ for $\SC$ is
$\Assum_\SC[t]\defeq\Assum_\MC[t]\land\Assum_{\hm}(\SC)[t]$. The reason that
$\Assum_\SC[t]\Rightarrow\Assum_\MC[t]$ is that $\SC$ uses merged staking and hence
...
$\SC$ are secure (i.e., when $\Assum_\MC\land\Assum_\SC$ holds), and when only $\MC$ is secure while the security assumption of
...
$\Assum_\MC$ and $\Assum_\SC$, and so examining this
...
\begin{lemma}[Persistence and Liveness]\label{lem:perslive}
  ...
  the assumptions $\Assum_\SC, \Assum_\MC$. For all slots $t$, if
  $\Assum_\SC[t]$ (resp. $\Assum_\MC[t]$) holds, then $\SC$ (resp. $\MC$)
...
\begin{lemma}\label{lem:firewall-tx-types}
  ...
  $$
    \LState =
    \merge\left(
        \{\LState_\MC^{\cup}[t], \LState_\SC^{\cup}[t]\}
    \right)
    \not\in
    \adalang
  %\; .
...
\begin{proof}
  ...
  an $\SC$
  maintainer $\party_\SC$, such that
  $\LState_{\MC}^{\party_\MC}[t] = \LState_{\MC}^{\cup}[t]$
  and
  $\LState_{\SC}^{\party_\SC}[t] = \LState_{\SC}^{\cup}[t]$,
  respectively.
  ...
  adopted by $\party_\SC$ at slot~$t$ (and recall that $\party_\SC$ maintains
  both chains). Let $\Chainmc'$ be the chain adopted by
  $\party_\MC$ at slot $t$. As before,
  $\textsc{annotatetx}_\SC(\Chainmc,\Chainsc)$ must be a fixpoint of
  $\textsc{verifytx}_\SC$ (as $\textsc{verifytx}_\SC$ checks for a fixpoint).
  ...
  by $\textsc{verifytx}_\SC$ on input
  $\textsc{annotatetx}_\SC(\Chainmc,\Chainsc)$, we deduce that
  ...
  the output of $\merge\left(
      \{\LState_\MC^{\party_\MC}[t], \LState_\SC^{\party_\SC}[t]\}
  \right)$.
  Due to the
...
\begin{lemma}[Firewall]\label{lem:firewall}
  ...
  $\Assum_\MC, \Assum_\SC$ with overwhelming probability in $k$.
\end{lemma}
\begin{proof}
  ...
  $(\party_\MC, \party_\SC)$ of honest parties such that $\party_\MC$ is a $\MC$
  maintainer and $\party_\SC$ is a $\SC$ maintainer such that
  $\LState_\MC^{\party_\MC}[t] = \LState_\MC^\cup[t]$ and
  $\LState_\SC^{\party_\SC}[t] = \LState_\SC^\cup[t]$ (if such parties exist).
  Let $\LState_1 = \LState_\MC^{\party_\MC}[t]$ and
  $\LState_2 = \LState_\SC^{\party_\SC}[t]$.
  ...
  a time slot $t$ and parties $\party_\MC, \party_\SC$, it returns \textsc{failure}
  ...
  of epoch $\epoch_{j-1}$ according to the view of $\party_\SC$
  ...
  the view of $\party_\SC$ into $\pending'_j$, and creates the respective
  ...
  $\party_\SC$, unless $j = 1$ in which case $avk^0$ is known.
  ...
  of honest parties such that $\party_\MC$ is a $\MC$ maintainer and $\party_\SC$ is
  ...
  parties $\party_\MC, \party_\SC$, it returns \textsc{failure}. Let $\tx$ be as in
  ...
  \textsc{failure}. When $\sccert_j$ was accepted by $\party_\SC$, $\pending_j$
  included a set of transactions $\vec\tx$ in the view of $\party_\SC$. If $\tx
  ...
  \noindent
  Because persistence holds in both $\MC$ and $\SC$, we know that there exist two parties $\party_\MC, \party_\SC$
  such that at slot $t$ we have that $\Ledger_\MC^{\party_\MC}[t] =
  \Ledger_\MC^{\cup}[t]$ and $\Ledger_\SC^{\party_\SC}[t] =
  \Ledger_\SC^{\cup}[t]$,
  respectively. Therefore $\scforge[t]$ implies
  $$
    \merge(\{
      \Ledger_\MC^{\party_\MC}[t],
      \Ledger_\SC^{\party_\SC}[t]
    \}) \not\in \adalang
  \; .
  $$
  ...
  $\Assum_\MC[t] \Rightarrow \Assum_\SC[t]$, we have $\Secure_t = \emptyset$ and
...
\begin{theorem}[Pegging Security]
  ...
  $u=2k$
  with respect to
  assumptions $\Assum_\MC$ and $\Assum_\SC$ defined above,
