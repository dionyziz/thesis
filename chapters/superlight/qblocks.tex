...
$(ctr,s,x)$ such that $H(ctr,s,x)\le T$.
Consider a block property $Q$. Let $\xi_Q=\Pr[Q(h)|h\le T]$, when $h$ is
...
As in \cite{EC:GarKiaLeo15}, the probability $f$ that at least one honest party
...
implicit in the proof of the Common-Prefix Lemma in \cite{EC:GarKiaLeo15} and was made
explicit in \cite{GKL15NEW}.
...
\begin{proof}
	...
	statement.
	Clearly, $r\in S$. Let us verify that $Y(S)\le Z(S)$.
	Indeed, if $\chain^*[v]$ is any block computed during a uniquely successful
	round $i\in S$, it must hold $u^*<v\le u'$. The first inequality is because
...
%\nikos{Requiring $t<(\frac12-\delta)(n-t)$ and $2f+2\eps\le\delta$ gives that
